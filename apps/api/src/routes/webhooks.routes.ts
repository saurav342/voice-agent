...
import { uploadRecordingToSupabase } from "../lib/storage.js";
...
webhooksRouter.post("/voicelink", async (req, res) => {
  ...
  if (typeof evt.event === "string" && evt.call && typeof evt.call === "object") {
    ...
    const statusMap: Record<string, "queued" | "ringing" | "inprogress" | "completed" | "failed"> = {
      ...
      "in-progress": "inprogress", completed: "completed", ended: "completed",
      failed: "failed", "no-answer": "failed", busy: "failed", canceled: "failed",
    };
    const status = statusMap[String(c.status)] ?? "ringing";
    const normalizedCallId = c.id ? denormalizeProviderCallId(c.id) : "";
    await db.collection("call_events").insertOne({
      providerCallId: normalizedCallId, eventType: String(evt.event), callStatus: c.status ?? null,
      hangupCause: c.hangupCause ?? null, receivedAt: now, rawPayload: raw as Record<string, unknown>,
    });
    const rawRecordingUrl =
      (c as any).recording_path ||
      (c as any).recording_url ||
      (c as any).recordingUrl ||
      (raw as any).recording_path ||
      (raw as any).recording_url;
    ...
    if (normalizedCallId) {
      ...
      if (typeof rawRecordingUrl === "string" && rawRecordingUrl.trim().length > 0) {
        uploadRecordingToSupabase(normalizedCallId, rawRecordingUrl)
          .then(async (uploadedUrl) => {
            if (uploadedUrl !== rawRecordingUrl) {
              await db.collection("calls").updateOne(
                { providerCallId: normalizedCallId },
                { $set: { recordingUrl: uploadedUrl, updatedAt: new Date() } }
              );
              log.info({ providerCallId: normalizedCallId, uploadedUrl }, "Updated call document with Supabase recording URL (v2)");
            }
          })
          .catch((err) => {
            log.error({ err, providerCallId: normalizedCallId }, "Error in background recording upload to Supabase (v2)");
          });
      }
    }

    log.info(
      { providerCallId: normalizedCallId, event: evt.event, status, hangupCause: c.hangupCause ?? null },
  ...
  const normalizedCallId = event.unique_id ? denormalizeProviderCallId(event.unique_id) : "";
  ...
  if (normalizedCallId) {
    ...
    // Fire background upload if recording exists
    if (callDoc.set.recordingUrl) {
      const originalUrl = callDoc.set.recordingUrl;
      uploadRecordingToSupabase(normalizedCallId, originalUrl)
        .then(async (uploadedUrl) => {
          if (uploadedUrl !== originalUrl) {
            ...
            log.info({ providerCallId: normalizedCallId, uploadedUrl }, "Updated call document with Supabase recording URL (v1)");
          }
        })
        .catch((err) => {
          log.error({ err, providerCallId: normalizedCallId }, "Error in background recording upload to Supabase (v1)");
        });
    }
    ...
    if (
      ...
      try {
        ...
        log.warn(
          { err, providerCallId: normalizedCallId, tenantId: resolvedTenantId },
