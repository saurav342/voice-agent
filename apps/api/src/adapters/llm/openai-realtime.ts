...
export class OpenAiRealtimeProvider implements RealtimeProvider {
  ...
  private userTranscriptHandlers: ((text: string, finished: boolean) => void)[] = [];
  ...
  private handleMessage(raw: string): void {
    let msg: { type?: string; delta?: string; audio?: string; transcript?: string };
    try {
      msg = JSON.parse(raw);
    } catch {
      return;
    ...
    switch (msg.type) {
      ...
      case "conversation.item.input_audio_transcription.completed":
        if (msg.transcript) {
          for (const h of this.userTranscriptHandlers) h(msg.transcript, true);
        }
        break;
      case "error":
  ...
  onUserTranscript(h: (text: string, finished: boolean) => void) { this.userTranscriptHandlers.push(h); }
